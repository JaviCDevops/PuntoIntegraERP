...
export default function Dashboard({ auth, kpis, chart, alertas, ausentes_list = [] }) {
    ...
    // Validamos que existan datos antes de renderizar gráficos o lógica compleja
    const hasAlerts = (alertas?.rrhh?.length > 0) || (alertas?.proyectos?.length > 0);

    const chartOptions = {
        responsive: true,
        maintainAspectRatio: false, // Permite que el gráfico se adapte mejor a la altura del contenedor
        plugins: {
            ...
            <div className="py-8">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        {/* KPI VENTAS */}
                        <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-indigo-500 relative transition hover:shadow-md">
                        ...
                        {/* KPI PROYECTOS */}
                        <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-emerald-500 relative transition hover:shadow-md">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Proyectos Activos</p>
                                    <h3 className="text-2xl font-bold text-gray-800 mt-1">{kpis?.proyectos_activos || 0}</h3>
                        ...
                        {/* KPI EQUIPO (MEJORADO CON LISTA DE AUSENTES) */}
                        <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-purple-500 relative transition hover:shadow-md">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Equipo Activo</p>
                                    <h3 className="text-2xl font-bold text-gray-800 mt-1">{kpis?.empleados_activos || 0}</h3>
                                    {kpis?.ausentes_hoy > 0 ? (
                                        <span className="text-xs text-red-500 font-bold mt-2 inline-block">⚠ {kpis.ausentes_hoy} ausente(s)</span>
                                    ) : (
                                        <span className="text-xs text-green-600 font-bold mt-2 inline-block">✓ Asistencia completa</span>
                                    )}
                                </div>
                            ...
                            {/* Lista de ausentes (NUEVO) */}
                            {ausentes_list && ausentes_list.length > 0 && (
                                <div className="mt-3 pt-3 border-t border-purple-100">
                                    <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Detalle Ausencias:</p>
                                    <ul className="space-y-1">
                                        {ausentes_list.map((a, idx) => (
                        ...
                        {/* KPI PENDIENTES */}
                        <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border-l-4 border-orange-400 relative transition hover:shadow-md">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-gray-500 text-xs font-bold uppercase tracking-wider">Por Gestionar</p>
                                    <h3 className="text-2xl font-bold text-gray-800 mt-1">{kpis?.cotizaciones_pendientes || 0}</h3>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                                    ...
                                    <div className="space-y-2">
                                        {alertas.rrhh.map(req => (
                                            <div key={req.id} className="flex justify-between items-center p-2 bg-red-50 rounded border border-red-100">
                                                <div>
                                                    <span className="text-sm font-bold text-gray-800">{req.employee?.user?.name || 'Empleado'}</span>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        {alertas.proyectos.map(alerta => {
                                            // LÓGICA CORREGIDA: Redondear los días
                                            const diasRedondeados = Math.round(alerta.days_diff);
                                            const esAtraso = diasRedondeados < 0;
                                            ...
                                            return (
                                                <div key={alerta.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-xs text-gray-500">Entrega: {alerta.deadline}</p>
                                                    </div>
                                                    <div className={`ml-2 px-2 py-1 text-[10px] font-bold text-white rounded whitespace-nowrap 
                                                        ${esAtraso ? 'bg-red-500' : 'bg-yellow-500'}`}>
                                                        {esAtraso 
                                                            ? `${Math.abs(diasRedondeados)} Días ATRASO` 
                                                            : `${diasRedondeados} Días Restantes`}
                                                    </div>
                                                </div>
                                            );
                                        })}
                            ...
                            {/* Estado sin alertas (Lógica corregida) */}
                            {!hasAlerts && (
